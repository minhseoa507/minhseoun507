...
URL_LOGIN = "https://gpoesteso.com/index/user/login"
INVITE_CODE = "268268"
NUM_WORKERS = 3
PASSWORD = "456789"
...
async def register_forever(index, browser):
    while True:
        ...
        try:
            ...
            # Click đăng ký bằng Playwright thật
            ...
            # Thử login ngay để xác nhận thành công
            ...
            if "/dashboard" in current_url or "index/user/index" in current_url:
                print(f"[luồng {index}] ✅ Đăng ký + đăng nhập thành công: {username}")
            else:
                print(f"[luồng {index}] ⚠️ Có thể đăng ký thành công nhưng login fail: {username}")
                await page.screenshot(path=f"errorshots/luong{index}_{username}_check_login.png")
